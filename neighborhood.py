...
def get_neighboring_docs(dataset, doc, max_subset_size, softmax_temp=.1):
    ...
    masks = get_all_masks(len(doc), max_subset_size)
    ...
    return neighborhood_tokens, neighborhood_perturbations
...
def choose_new_tokens(token, token_vector, tag_matrix, tag_vocab,
    ...
    token_distances = softmax(all_similarities[most_similar_indices], softmax_temp)

    selected_indices = np.random.choice(np.arange(token_distances.size),
                                        size=n_samples, p=token_distances)
